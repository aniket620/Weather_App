import pymongo   #used for PyMongo distribution contains tools for interacting with MongoDB database from Python
...
city_names = Configuration.locations

# How often do we want our app to refresh and download data
refresh_frequency = Configuration.refresh_frequency

#use this thread to download 5days /3 hour forecast.Also show alert if there is any alert for rain/snow.
def thread_for_5_days_3_hour_forecast():

    #initialize Mongodb client running on locahost
...
def thread_for_weather_maps(locations):
 ...
 #           folium.TileLayer('cartodbpositron').add_to(maps)
            ...
            maps.save("C://Users//Owner//Desktop//Weather_map_data//"+city+"_"+last_timestamp_date.replace(":","_")+".html")
...
#        folium.TileLayer('cartodbdark_matter').add_to(maps)
        ...
        legend_html =   '''
                ...
                ''' %(icon_url,weather)
        maps.get_root().html.add_child(folium.Element(legend_html))        
        maps.save("C://Users//Owner//Desktop//Weather_map_data//Latest_weather_map"+city+"_"+last_timestamp_date.replace(":","_")+"_latest_weather.html")
...
def thread_for_plotting_forecast_data():
    ...
    for city in city_names:
        ...
        plt.savefig("C://Users//Owner//Desktop//Weather_map_data//Plots//"+city+".png")
        plt.close()
